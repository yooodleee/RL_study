...
class CsvWriter:
    ...
    Each `write()` takes a `OrderDict`, creating one column in the CSV file for
        each dictionary key on the first call. Successive calls to `write()` 
    ...
    def write(
            self, values: collections.OrderedDict) -> None:
        """
        ...
        # the same file after e.g. restarting from a checkpoint.
        with open(
