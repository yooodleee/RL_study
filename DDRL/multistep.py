...
def n_step_bellman_target(
    ...
    See section 2.3 of R2D2 paper (which does not mention the logic around end 
        of episode).

    Args:
        reward: This is r_t in the equations below. Should be non-discounted, 
            non-summed, shape [T, B] tensor.
        ...
        q_t: This is Q_target(s_{t+1}, a*) (where a* is an action chosen by 
            the caller), shape [T, B] tensor.
        gamma: Exponential RL discounting.
        n_steps: The number of steps to loock ahead for computing the Bellman 
            targets.
    ...
    Returns:
        ...
        The last n_step-1 targets cannot be computed with n_step returns, since 
            run out of Q_{target}(s_{t+n}). Instead, they will use n_step-1, ..
            , 1 step returns. For those last targets, the last Q_{target}
    ...
    # prepend 0s that will be discarded at the first iteration below.
    ...
    done = torch.concat([done] 
                        + [torch.zeros_like(done[0:1])] 
                        * n_steps, dim=0)
    rewards = torch.concat([r_t] 
                           + [torch.zeros_like(r_t[0:1])] 
                           * n_steps, dim=0)
...
def truncated_generalized_advantage_estimation(
    ...
    """
    Computes truncated generalized advantage estimates for a sequence length 
        k.

    The advantages are computed in a backwards fashion according to the 
        equation:
    ...
    Returns:
        Multistep truncated generalized advantage estimation at times 
            [0, k-1].
    """
...
def general_off_policy_returns_from_action_values(
        ...
        c_t: torch.Tensor,
        pi_t: torch.Tensor) -> torch.Tensor:
    """
    ...
    Given a window of experience of length `k` generated by a behavior policy 
        ...
        onwards, under some target policy π, using the rewards in the 
        trajectory, the actions selected by μ, using the rewards in the 
        trajectory, the actions selected by μ and the action-values under π, 
    ...
    Args:
        ...
        pi_t: target policy probs at times [1, ..., k - 1], shape 
            [T, B, action_dim].

    Returns:
        off-policy estimates of the generalized returns from states visited at 
            times [0, ..., k - 1], shape [T, B].
    """
...
def general_off_policy_returns_from_q_and_v(
    ...
    Given a window of experience of length `k+1`, generated by a behavior 
        policy
        μ, for each time-step `t` we can estimate the return `G_t` from that 
        step onwards, under some target policy π, using the rewards in the 
        trajectory, the values under π of states and actions selected by μ, 
        according to equation:
    ...
    Args:
        ...
        stop_target_gradients: bool indicating whether or not to apply stop 
            gradient to targets.
    
    Returns:
        off-policy estimates of the generalized returns from states visited 
            at times [0, ..., k - 1].
    """
