...
class Agent57Transition(NamedTuple):
    ...
    a_t: Optional[int]
    # q values for s_t, computed from both ext_q_network and int_q_network
    q_t: Optional[np.ndarray]   
    ...
    # nn.LSTM initial hidden state, from ext_q_network
    ext_init_h: Optional[np.ndarray]    
    # nn.LSTM initial cell state, from ext_q_network
    ext_init_c: Optional[np.ndarray]    
    # nn.LSTM initial hidden state, from in_q_network
    int_init_h: Optional[np.ndarray]    
    # nn.LSTM initial cell state, from int_q_network
    int_init_c: Optional[np.ndarray]    
...
class Agent(types_lib.Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            unroll_length: how many agent time step to unroll transitions 
                before put on to queue.
            ...
            cluster_distance: K-NN neighbors cluster distance for compute 
                episodic bonus.
            ...
            shared_params: a shared dict, so we can later update the 
        ...
        if not 1 <= unroll_length:
            raise ValueError(
                f'Expect unroll_length to be integer greater than or equal to 
                1, got {unroll_length}')
        if not 0 <= burn_in < unroll_length:
            raise ValueError(
                f'Expect burn_in length to be [0, {unroll_length}), got 
                {burn_in}')
        if not 1 <= num_policies:
            raise ValueError(
                f'Expect num_policies to be integer greater than or equal to 1
                , got {num_policies}')
        ...
        if not 1 <= ucb_window_size:
            raise ValueError(
                f'Expect ucb_window_size to be integer greater than or equal 
                to 1, got {ucb_window_size}')
        ...
        if not 1 <= episodic_memory_capacity:
            raise ValueError(
                f'Expect episodic_memory_capacity to be integer greater than 
                or equal to 1, got {episodic_memory_capacity}')
        if not 1 <= num_neighbors:
            raise ValueError(
                f'Expect num_neighbors to be integer greater than or equal to 
                1, got {num_neighbors}')
        if not 0.0 <= cluster_distance:
            raise ValueError(
                f'Expect cluster_disctance to be [0.0, 1.0], got 
                {cluster_distance}')
        if not 0.0 <= kernel_epsilon <= 1.0:
            raise ValueError(
                f'Expect kerenl_epsilon to be [0.0, 1.0], got 
                {kernel_epsilon}')
        if not 1 <= actor_update_interval:
            raise ValueError(
                f'Expect actor_update_interval to be integer greater than or 
                equal to 1, got {actor_update_interval}')
        ...
        # Disable autograd for actor's Q networks, embedding, and RND 
        # networks.
        ...
        self._unroll = replay_lib.Unroll(
            ...
            # Plus 1 to add room for shift during learning
            overlap=burn_in + 1,    
        ...
        # Stores nn.LSTM hidden state and cell state. for extrinsic Q network
        self._ext_lstm_state = None 
        # Stores nn.LSTM hidden state and cell state. for intrinsic Q network
    ...
    def step(self, timestep: types_lib.TimeStep) -> types_lib.Action:
        """
        Given timestep, return action a_t, and push transition into global 
            queue
        """
        self._step_t += 1
        ...
        transition = Agent57Transition(
                        ...
                        ext_init_h=self._ext_lstm_state[0].\
                            squeeze(1).cpu().numpy(),    
                        ext_init_c=self._ext_lstm_state[1].\
                            squeeze(1).cpu().numpy(),
                        # remove batch dimension
                        int_init_h=self._int_lstm_state[0].\
                            squeeze(1).cpu().numpy(),    
                        int_init_c=self._int_lstm_state[1].\
                            squeeze(1).cpu().numpy())

        unrolled_transition = self._unroll.add(
                                transition, timestep.done)

        s_t = torch.from_numpy(timestep.observation[None, ...]).\
    ...
    def reset(self) -> None:
        ...
        self._meta_coll.update(self._policy_index, self._episodic_returns)

        self._episodic_returns = 0.0

        # Agent57 actor samples a policy using the Sliding Window UCB 
        # algorithm, then play a single episode.
        self._sample_policy()
        ...
        self._ext_lstm_state, self._int_lstm_state = self._network.\
            get_initial_hidden_state(batch_size=1)
    
    def act(self, timestep: types_lib.TimeStep) \
        -> Tuple[
                np.ndarray,
                types_lib.Action,
                float,
    ...
    @torch.no_grad()
    def _choose_action(self, timestep: types_lib.TimeStep) \
        -> Tuple[
                np.ndarray,
                types_lib.Action,
                float,
    ...
    def _prepare_network_input(
            self, timestep: types_lib.TimeStep) -> Agent57NetworkInputs:
...
class Learner(types_lib.Learner):
    ...
    def __init__(
        ...
        Args:
            ...
            burn_in: burn n transitions to generate initial hidden state 
                before learning.
            ...
            priority_eta: coefficient to mix the max and mean absoulte TD 
                errors.
            clip_grad: if True, clip gradients norm.
            max_grad_nrom: the maximum gradient norm for clip grad, only works 
        ...
        if not 1 <= target_net_update_interval:
            raise ValueError(
                ...
                got {target_net_update_interval}')
        if not 1 <= min_replay_size:
            raise ValueError(
                f'Expect min_replay_size to be integer greater than or equal 
                to 1, got {min_replay_size}')
        if not 1 <= batch_size <= 128:
            raise ValueError(
                f'Expect batch_size to in the range [1, 128], got 
                {batch_size}')
        if not 1 <= unroll_length:
            raise ValueError(
                f'Expect unroll_length to be greater than or equal to 1, got
                {unroll_length}')
        if not 0 <= burn_in < unroll_length:
            raise ValueError(
                f'Expect burn_in length to be [0, {unroll_length}), got 
        ...
        if not 0.0 <= retrace_lambda <= 1.0:
            raise ValueError(
                f'Expect retrace_lambda to in the range [0.0, 1.0], got 
                {retrace_lambda}')
        if not 0.0 <= priority_eta <= 1.0:
            raise ValueError(
                f'Expect priority_eta to in the range [0.0, 1.0], got 
                {priority_eta}')
        ...
        self._target_network = copy.deepcopy(self._netowrk).to(device=device)
    ...
    def received_item_from_queue(self, item) -> None:
        ...
        """
        self._replay.add(item, self._max_see_priority)
    
    def get_network_state_dict(self):
        """
        To keep things consistent, move the parameters to CPU.
        """
        return {k: v.cpu() for k, v in self._netowrk.state_dict().items()}
    ...
    def get_embedding_network_state_dict(self):
        ...
        return {
            k: v.cpu() 
            for k, v 
            in self._embedding_network.state_dict().items()}
    ...
    def get_rnd_predictor_network_state_dict(self):
        ...
        return {
            k: v.cpu() 
            for k, v 
            in self._rnd_predictor_network.state_dict().items()}
    ...
    def _update_q_network(
        ...
        # Get initial hidden state for batch extrinsic and intrinsic Q 
        # networks, possible burn in.
        ...
        if burn_transitions is not None:
            ...
            ext_hidden_s = tuple(s.clone().to(device=self._device) 
                                 for s in init_ext_hidden_s)
            target_ext_hidden_s = tuple(s.clone().to(device=self._device) 
                                 for s in init_ext_hidden_s)
            # Make copy of hidden state for intrinsic Q networks.
            int_hidden_s = tuple(s.clone().to(device=self._device)
                                 for s in init_int_hidden_s)
            target_int_hidden_s = tuple(s.clone().to(device=self._device)
                                 for s in init_int_hidden_s)
    ...
    def _get_predicted_q_values(
        ...
        """
        s_t = torch.from_numpy(transitions.s_t).\
                to(device=self._device, dtype=torch.float32)  # [T+1, B, state_shape]
        last_action = torch.from_numpy(transitions.last_action).\
                        to(device=self._device, dtype=torch.int64) # [T+1, B]
        ext_r_t = torch.from_numpy(transitions.ext_r_t).\
                    to(device=self._device, dtype=torch.float32)  # [T+1, B]
        int_r_t = torch.from_numpy(transitions.int_r_t).\
                    to(device=self._device, dtype=torch.float32)  # [T+1, B]
        policy_index = torch.from_numpy(transitions.policy_index).\
                        to(device=self._device, dtype=torch.int64) # [T+1, B]
    ...
    def _calc_retrace_loss(
            ...
            transitions: Agent57Transition,
            q_t: torch.Tensor,
            target_q_t: torch.Tensor) -> Tuple[torch.Tensor, np.ndarray]:
        
        a_t = torch.from_numpy(transitions.a_t).\
                to(device=self._device, dtype=torch.int64)    # [T+1, B]
        behavior_prob_a_t = torch.from_numpy(transitions.prob_a_t).\
                                to(device=self._device, dtype=torch.float32)  # [T+1, B]
        ext_r_t = torch.from_numpy(transitions.ext_r_t).\
                    to(device=self._device, dtype=torch.float32)  # [T+1, B]
        int_r_t = torch.from_numpy(transitions.int_r_t).\
                    to(device=self._device, dtype=torch.float32)  # [T+1, B]
        beta = torch.from_numpy(transitions.beta).\
                to(device=self._device, dtype=torch.float32)  # [T+1, B]
        discount = torch.from_numpy(transitions.discount).\
                    to(device=self._device, dtype=torch.float32)  # [T+1, B]
        ...
        # [T+1, B, action_dim]
        target_policy_probs = F.softmax(target_q_t, dim=-1)   
    ...
    def _update_embed_and_rnd_predictor_networks(
        ...
        """
        b = self._batch_size
        weights = torch.from_numpy(weights[-b:]).\
    ...
    def _calc_rnd_loss(
            self,
            transitions: Agent57Transition) -> torch.Tensor:
        # [5, B, state_shape]
        s_t = torch.from_numpy(transitions.s_t[-5:]).\
                to(device=self._device, dtype=torch.float32)  
    ...
    def _calc_embed_inverse_loss(
            ...
            transitions: Agent57Transition) -> torch.Tensor:
        # [6, B, state_shape].
        s_t = torch.from_numpy(transitions.s_t[-6:]).\
                to(device=self._device, dtype=torch.float32)  
        a_t = torch.from_numpy(transitions.a_t[-6:]).\
        ...
        # [5*B, action_dim]
        pi_logits = self._embedding_network.inverse_prediction(embeddings)  
    ...
    def _burn_in_unroll_q_networks(
        ...
        # Rank and dtype checs, note have a new unroll time dimension, states 
        # may be images, which is rank 5.
        base.assert_rank_and_dtype(s_t, (3, 5), torch.float32)
        base.assert_rank_and_dtype(last_action, 2, torch.long)
        base.assert_rank_and_dtype(ext_r_t, 2, torch.float32)
        base.assert_rank_and_dtype(int_r_t, 2, torch.float32)
        ...
        _ext_hidden_s = tuple(s.clone().to(device=self._device) 
                              for s in ext_hidden_state)
        _int_hidden_s = tuple(s.clone().to(device=self._device) 
                              for s in int_hidden_state)
        _target_ext_hidden_s = tuple(s.clone().to(device=self._device) 
                                     for s in ext_hidden_state)
        _target_int_hidden_s = tuple(s.clone().to(device=self._device) 
                                     for s in int_hidden_state)

        # Burn in to generate hidden state for LSTM, unroll both online and 
        # target Q networks
        output = network(
                    Agent57NetworkInputs(
                        s_t=s_t,
    ...
    def _extract_first_step_hidden_state(
            self,
            transitions: Agent57Transition) -> Tuple[HiddenState, HiddenState]:
        """
        ...
        ext_init_h = torch.from_numpy(transitions.ext_init_h[0:1]).\
                        squeeze(0).to(device=self._device, dtype=torch.float32)
        ext_init_c = torch.from_numpy(transitions.ext_init_c[0:1]).\
                        squeeze(0).to(device=self._device, dtype=torch.float32)
        int_init_h = torch.from_numpy(transitions.int_init_h[0:1]).\
                        squeeze(0).to(device=self._device, dtype=torch.float32)
        int_init_c = torch.from_numpy(transitions.int_init_c[0:1]).\
                        squeeze(0).to(device=self._device, dtype=torch.float32)
