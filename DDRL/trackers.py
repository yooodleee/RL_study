...
class EpisodeTracker:
    ...
    def step(
        ...
        if timestep_t.first:
            ...
            # Try to use the non-clipped/unscaled raw reward when collecting 
            # statistics
            if isinstance(timestep_t.info, dict) \
    ...
    def get(self) -> Mapping[str, Union[int, float, None]]:
        ...
        Here the convention is `episode_return` is set to 
            `current_episode_return`
            if a full episode has not been encountered. Otherwise it is set to
            `mean_episode_return` which is the mean return of complete 
                episodes only.
            If no steps have been taken at all, `episode_return` is set to 
                `NaN`.
        ...
        if len(self._episode_returns) > 0:
            ...
            if len(self._episode_visited_rooms) > 0:
                mean_episode_visited_rooms = np.array(
                    self._episode_visited_rooms).mean()
        else:
...
class TensorboardEpisodeTracker(EpisodeTracker):
    ...
    def __init__(self, writer: SummaryWriter):
        super().__init__()
        # Keep track total number of steps, does not reset
        self._total_steps = 0   
        # Keep track total number of episodes, does not reset
...
class TensorboardScreenshotTracker:
    """
    Take screenshots of the environment and add to tensorboard every N 
    episodes. This should be used for debugging only.
    """
...
class TensorboardLearnerStatisticsTracker:
    ...
    def step(self, stats) -> None:
        ...
        if self._total_steps % 100 == 0:
            ...
            # This should not block the training loop if there's any 
            # exception.
            try:
                if stats:
                    for k, v in stats.items():
...
def make_default_trackers(
    ...
    Args:
        ...
        debug_screenshots_interval: the frequeny to take screenshots and add 
...
def generate_statistics(trackers: Iterable[Any],
        timestep_action_sequence: Iterable[
            Tuple[Optional[replay_lib.Transition]]]) -> Mapping[str, Any]:
    """
