...
class PyTorchCheckpoint:
    ...
    Exmaple create checkpoint:
        Create checkpoint instance register network to the checkpoint 
            ...
            checkpoint = PyTorchCheckpoint(
                ...
                agent_name='NGU',
                save_dir=FLAGS.checkpint_dir,
            )
            checkpoint.register_dir(('network', network))
    ...
    Example restore checkpoint:
        Create checkpoint instance and register network to the checkpoint 
            ...
            checkpoint.register_pari(('network', network))
    ...
    def __init__(
        ...
        if not restore_only and self.save_dir is not None \
            and self.save_dir != '':
            self.baes_path = Path(self.save_dir)
            if not self.baes_path.exists():
                self.baes_path.mkdir(parents=True, exist_ok=True)
    ...
    def save(self) -> str:
        ...
        Save pytorch model as checkpoint, default file name is
            ...
            for example A2C_CartPole-v1_0.ckpt.
    ...
    def restore(self, file_to_restore: str) -> None:
        ...
        """
        if not file_to_restore or not os.path.isfile(file_to_restore) \
            or not os.path.exists(file_to_restore):
            raise ValueError(
                f'"{file_to_restore}" is not a valid checkpoint file.'
            )
        ...
        if loaded_state['environment_name'] != self.state.environment_name:
            err_msg = f'environment_name "{loaded_state["environment_name"]}" '\
                    f'and "{self.state.environment_name}" mismatch.'
            raise RuntimeError(err_msg)
        
        if 'agent_name' in loaded_state and loaded_state['agent_name'] \
            != self.state.agent_name:
            err_msg = f'agent_name "{loaded_state["agent_name"]}" '\
                    f'and "{self.state.agent_name}" mismatch.'
            raise RuntimeError(err_msg)
