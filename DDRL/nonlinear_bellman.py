...
# Example transform pairs; these typically consist of a monotonically 
# increasing squashing fn `apply` and its inverse `apply_inv`. Ohter choices 
# are possible.
...
def transformed_retrace(
        ...
        lambda_: float,
        eps: float = 1e-8,
        tx_pair: TxPair = IDENTITY_PAIR) -> base.LossOutput:
    """
    calculates transformed Retrace errors.

    See "Recurrent Experience Replay in Distributed Reinforcement Learning" 
        by Kapturowski et al. (https://openreview.net/pdf?id=r1lyTjAqYX).
    ...
    c_t = torch.minimum(torch.tensor(1.0),
            pi_a_t / (mu_t + eps)) * lambda_
