"""
    ...
    Some of the complexity is captured via wrappers but the main components such as the DQN model itsef,
    the training loop, the momort-efficient replay buffer are implemented from scratch.
...
class ActorLearner:
    ...
    def learn_from_experience(self):
        ...
        with torch.no_grad():
            ...
            next_state_max_q_values=self.target_dqn(next_states).max(dim=1, keepdim=True)
        ...
        if self.learner_cnt % self.target_dqn_update_interval == 0:
            ...
            else:   # soft update, the 2 branches can be merged together, leaving it like this for new
    ...
    def maybe_log_episode(self):
        ...
        if rewards[-1] > self.best_episode_reward:
            ...
            self.best_episode_reward=copy.deepcopy(self.dqn)    # keep track of the model that gave the best reward
    ...
    def maybe_log(self):
        ...
        if self.grads_log_freq is not None and self.learner_cnt > 0 and self.learner_cnt % self.grads_log_freq == 0:
            ...
            self.tensorboard_writer.add_scalar(f"grad_norms/total", total_grad_12_norm, self.learner_cnt)
    
    def log_to_console(self):   # keep it minimal for now, I mostly use toesorboard - feel free to expand functionality
...
def train_dqn(config):
    ...
    dqn=DQN(env, number_of_actions=env.action_space.n, epsilon_shcedule=linear_schedule).to(device)
