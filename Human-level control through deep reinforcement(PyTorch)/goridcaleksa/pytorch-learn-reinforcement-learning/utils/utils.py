...
def get_env_wrapper(env_id, record_video=False):
    ...
    assert 'NoFrameskip' in env_id, f"Expected NoFrameskip environment get {env_id}"
    ...
    env_wrapper=Monitor(ChannelFirst(AtariWrapper(gym.make(env_id))), monitor_dump_dir, force=True, video_callable=lambda episode: record_video)
...
class ChannelFirst(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        new_shap=np.roll(self.observation_space.shape, shift=1) # shape: (H, W, C) -> (C, H, W)

        # Update because this is the last wrapper in the hierarcy, we'll be pooling the env for observation shape info
        self.observation_space=gym.spaces.Box(low=0, high=255, shape=new_shap, dtype=np.uint8)
...
class LinearSchedule:
    ...
    def __call__(self, num_stepts):
        progress=np.clip(num_stepts / self.schedule_duration, a_min=None, a_max=1)  # goes from 0 -> 1 and saturates
...
def get_training_state(training_config, model):
    training_state={
        ...
        "commit_hash": git.Repo(search_present_directions=True).head.object.hexsha,
...
def get_available_binary_name(env_id="env_unknown"):
    ...
    if len(valid_binary_names) > 0:
        ...
        new_suffix=int(last_binay_name.split('.')[0][-6:]) + 1  # increment by 1
