...
class ReplayBuffer:
    ...
    """
    def __init__(self, max_buffer_size, num_last_frames_to_fatch=4, frame_shape=[1, 84, 84], crash_if_no_mem=True):
        ...
        # Create main buffer contains - all types are chosen so as to optimize memory consumption
        ...
        # Numpy dones lazy execution so it can happen that only after a while the training starts hitting the RAM limit
        # than the page swapping kicks in which will slow-down the training significantly only after hours of training!
        # hance the _check_eough_ram function (I'm saving you time and money thank me later ^^)
    ...
    def fetch_random_states(self, batch_size):
        ...
        next_states=self._postprocess_state(
            np.concatenate([self._fetch_state(i+2) for i in random_unique_indices], 0)  # shape = (B, C, H, W)
        )
        ...
        rewards=torch.from_numpy(self.actions[random_unique_indices]).to(self.device)
    ...
    def _fetch_state(self, end_index):
        ...
        num_of_missing_frames=self.num_previous_frames_to_fatch - (end_index - start_index)
        ...
        if start_index < 0 or num_of_missing_frames > 0:    # start_index : end_index indexing won't work if start_index < 0
            ...
            # original DeepMind Lua imp: https://github.com/deepmind/dqn/blob/master/dqn/TransitionTable.lua#171
            ...
            # return from (C, 1, H, W) to (1, C, H, W) where C number of past frames, 4 for Atari
    ...
    def _handle_start_index_edge_cases(self, start_index, end_index):
        ...
        if self._buffer_full():
            if 0 < (self.current_free_slot_index - start_index) % self.max_buffer_size < self.num_previous_frames_to_fatch:
    ...
    def _has_enough_data(self, batch_size):
        return batch_size < self.current_buffer_size    # e.g. if buffer size is 32 we need at least 32 frames hence <
    ...
    def _check_enough_ram(self, crash_if_no_mem):
        ...
        print(f'required memory = {to_GBs(required_memory)} GBs, available memory = {to_GBs(available_memory)} GB')
...
if __name__ == "__main__":
    ...
    # NoFrameskip - receive every frame from mthe env whereas the version without NoFrameskip would give every 4th frame
    # v4 - actions we send to env are ececuted, whereas v0 would ignore the last action we sent with 0.25 probability
    ...
    for i in range(num_of_collection_steps):
        ...
        print(f"Sampling action {random_action} - {env.unwrappered.get_action_meanings()[random_action]}")
