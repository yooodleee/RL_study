...
class DQN(nn.Module):
    ...
    I wrote the architecture a bit more generic, hence more lines of code,
        but it's more flaxible if you want to experiment with the DQN 
        architecture.
    """
    def __init__(
            self,
            env,num_in_chennels=4, 
            number_of_actions=3, 
            epsilon_shcedule=None):
        
        super().__init__()
        self.env = env
        # defines the annealing strategy for epsilon in epsilon-greedy
        self.epsilon_shcedule = epsilon_shcedule  
        # counts the number of calls to epsilon function
        self.num_in_chennels = num_in_chennels    

        #
        # CNN params - from the Nature DQN paper - MODIFY this part if you 
        # want to experiment
        #
        ...
        for i in range(len(num_of_filters_cnn) - 1):
            cnn_modules.extend(
                self._cnn_block(num_of_filters_cnn, 
                                num_of_filters_cnn[i + 1], 
                                kernal_sizes[i], strides[i]))
        
        self.cnn_part=nn.Sequential(
                        *cnn_modules,
                        # flatten from (B, C, H, W) into (B, C*H*W), where B 
                        # is batch size and c number of in channels
                        nn.Flatten())
        ...
        # automatically figure out the shape for the given env observation
        with torch.no_grad():   
            # shape = (1, C', H, W), unit8, where C' is originally 1, i.e. 
            # grayscale frames
            ...
            if dummy_input.shape[1] != num_in_chennels:
                ...
                dummy_input = dummy_input.repeat(
                                1, int(num_in_chennels \
                                       / dummy_input.shape[1]), 1, 1).float()
        ...
        for i in range(len(num_of_neurons_fc) -1):
            # last layer shouldn't have activation (Q-value is unbounded)
            last_layer= i == len(num_of_neurons_fc) -1  
            fc_modules.extend(
                self._fc_block(num_of_neurons_fc[i], 
                               num_of_neurons_fc[i + 1], 
                               use_relu=not last_layer))
    ...
    def epsilon_greedy(self, state):
        assert self.epsilon_shcedule is not None, \
            f"No schedule provided, can't call epsilon_greedy funtion"
        ...
        if np.random.rand() < self.epsilon_value():
            ...
            # Shape evolution: (1, C, H, W) -> (forward) (1, NA) -> (argmax) 
            # (1, 1) -> [0] scalar
    ...
    # The original CNN didn't use any padding: 
    # https://github.com/deepmind/dqn/blob/master/dqn/convnet.lua
    # not that it matters - it would probably work either way feel free to 
...
if __name__ == "__main__":
    # NoFrameskip - receive every frame from the env whereas the version 
    # without NoFrameskip would give every 4th frame
    # v4 - actions we send to env are excuted, whereas v0 would ignore the 
    # last action we sent with 0.25 probability
    env_id="PongNoFrameskip-v4"
    env_wrapped=get_env_wrapper(env_id)
    # testing only the __init__ function (mainly the automatic shape 
