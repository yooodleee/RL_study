...
def value_iteration(
    ...
    """
    Standard value iteration to compute optimal policies for the grid
        environments.
    ...
    S:      List of states
    A:      List of actions
    L:      Lableing function (it is a dictionary from states to events)
    ...
    returns
    ----------
    Optimal deterministic policy (dictionary mapping from states
        (SxU) to actions).
    """
    ...
    for s1 in S:
        for u1 in U:
            ...
            # argmax over the q_values
            a_i = max((x, i) for i, x in enumerate(q_values))[1]
            policy[(s1, u1)] = A[a_i]
