...
def learn(
    ...
    assert (np.abs(env.action_space.low) == env.action_space.high).all()    # assume symmetric actions.
    ...
    for epoch in range(nb_epochs):
        for cycle in range(nb_epoch_cycles):
            # Perform rollouts.
            if nenvs > 1:
                # If simulating multiple envs in parallel, impossible to reset agent at the end of the
                # episode in each of the environments, so resetting here instead
                agent.reset()
            ...
            for t_rollout in range(nb_rollout_steps):
                ...
                # max_action is of dimension A, whereas act is dimension (nenvs, A) - the multiplication 
                # gets broadcasted to the batch

                # scale for execution in env (as far as DPPG is concerned, every action is in [-1, 1])
                ...
                if nenvs == 1:
                    if not (use_crm or use_rs):
                        # Standart DDPG
                        # the batched data will be unrolled will be unrolled in memory.py's append.
                        ...
                        for _obs, _action, _r, _new_obs, _done in experiences:
                            ...
                            agent.store_transition(_obs, _action, _r, _new_obs, _done)
                else:
            ...
            for t_train in range(nb_train_steps):
                # Adaptive param noise, if necessary.
        ...
        # Log stats.
        # XXX shoudln't call np.mean on variable length lists
        ...
        combined_stats["rollout/return_history_std"] = np.mean(episode_rewards_history)
