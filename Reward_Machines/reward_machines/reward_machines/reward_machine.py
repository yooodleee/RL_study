...
class RewardMachine:
    ...
    def add_reward_shaping(self, gamma, rs_gamma):
        """It computes the potential values for shaping the reward function:
            - gamma(float):
                this is the gamma from the environment.
            - rs_gamma(float):
                this gamma that is used in the value iteration that compute 
                the shaping potentials.
        """
        self.gamma = gamma
        self.potentials = value_iteration(
            self.U, self.delta_u, self.delta_r, self.terminal_u, rs_gamma
        )
    ...
    def step(
        ...
        """Emulates an step on the reward machine from state *n1* when observing *true_props*.
        The rest of the params are for computing the reward when working with non-simple RMs:
            s_info (extra state information to compute the reward).
        """
    ...
    def get_useful_transitions(self, u1):
        """This is an auxiliary method used by the HRL baseline to prune
        "useless" options.
        """
        return [
            self.delta_u[u1][u2].split("&")
            for u2 in self.delta_u[u1]
            if u1 != u2 
        ]
    ...
    def _get_reward(
        ...
        rs = 0.0
        if add_rs:
            # If the env reached a terminal state, you have to use the potential from the terminal RM state 
            # to keep R5 optimality guarantees
            un = self.terminal_u if env_done else u2  
    ...
    def _load_reward_machine(self, file):
        ...
        for e in lines[2:]:
            ...
            # Adding state-transition to delta-u
            if u1 not in self.delta_u:
                self.delta_u[u1] = {}
            self.delta_u[u1][u2] = reward_function
