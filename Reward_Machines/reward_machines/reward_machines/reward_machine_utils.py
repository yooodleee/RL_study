...
def evaluate_dnf(formula, true_props):
    ...
    if "|" in formula:
        for f in formula.split("|"):
            if evaluate_dnf(f, true_props): return True
    ...
    if "&" in formula:
        for f in formula.split("&"):
            if not evaluate_dnf(f, true_props): return False
...
def value_iteration(U, delta_u, delta_r, terminal_u, gamma):
    """Standart value iteration approach. You use it to compute the potential function for
    the automated reward shaping.
    """
        ...
        for u1 in U:
            ...
            for u2 in delta_u[u1]:
                if delta_r[u1][u2].get_type() == "constant":
                    ...
                    # If the reward func is not constant, you assume it returns a reward of zero
                    r = 0
