...
It also keeps track of the RM state as the agent interacts with tne enviornment.

However, each environment must implement the following function:
    - *get_events(...)*: Returns the propositions that currently with the environment.
    
None:
    - The episode ends if the RM reaches a terminal state or the environment reaches a 
        terminal state.
...
class RewardMachineEnv(gym.Wrapper):
    ...
    def __init__(self, env, rm_files):
        ...
        # The observation spaces is a dictionary including the env features and a one-hot
        # representation of the state in the reward machine
        ...
        for rm_id, rm in enumerate(self.reward_machines):
            for u_id in rm.get_states():
                ...
                u_features[len(self.num_rm_states)] = 1
                self.rm_state_features[(rm_id, u_id)] = u_features
        self.rm_done_feat = np.zeros(self.num_rm_states)    # for terminal RM states, you give as features an array of zeros
    ...
    def reset(self):
        ...
        self.current_rm_id = (self.current_rm_id) % self.num_rms
    ...
    def step(self, action):
        ...
        # Getting the output of the detectors and saving information for generating
        ...
        # Update the RM state
        self.current_u_id, rm_rew, rm_done = self.current_rm.step(self.current_u_id,
                                                                  true_props,
                                                                  info)
        
        # Returning the result of this action
        done = rm_done or env_done
        rm_obs = self.get_observation(next_obs, self.current_rm_id, 
                                      self.current_u_id, done)
...
class RewardMachineWrapper(gym.Wrapper):
    ...
    def reset(self):
        """Use `valid_states`, set to compute RM states that are reachable by the last experience 
    ...
    def step(self, action):
        ...
        if self.add_crm:
            ...
            _, rs_rm_rew, _ = rm.step(u_id, rs_true_props, rs_info,
                                      self.add_rs, rs_env_done)
    ...
    def _get_rm_experience(
        ...
        for rm_id, rm in enumerate(self.reward_machines):
            for u_id in rm.get_states():
                exp, next_u = self._get_rm_experience(rm_id, rm, u_id, obs, action,
                ...
                if self.valid_states is None or (rm_id, u_id) in self.valid_states:
                    # only add experience that are possible (i.e., it is possible reach state u_id given
                    # the previous experience)
                    experiecnes.append(exp)
...
class HierarchicalRMWrapper(gym.Wrapper):
    ...
    Each option policy is rewarded when the current experience would have
    cause a transition through that edge.

    Methods
    ----------------
        - __init__(self, env, r_min, r_max, use_self_loops):
            - In addition of extracting the set of options available, it 
                initializes the following attributes:
                    - self.option_observation_space: 
                        spaces of options (concatenation of the env features
                        and the one-hot encoding of the option id)
                    - self.option_action_space:
                        space of actions wrt the set of available options
            - Parameters:
                - env(RewardmachineEnv):
                    It must be an RM environment.
                - r_min(float):
                    Reward given to the option policies when they failed to 
                    accomplish their goal.
                - r_max(float):
                    Reward given to the option policies when they accomplished 
                    their goal.
                - use_self_loops(bool):
                    When true, it adds option policies for each self-loop in the 
                    RM
                - add_rs(bool):
                    If true, it will add reward shaping to info
                - gamma(float):
                    Discount factor for the environment
                - rs_gamma(float):
                    Discount factor for shaping the rewards in the RM
        
        - get_valid_options(self):
            - Returns the set of valid options in the current RM state.
        - get_option_observation(self, option_id):
            - Returns the concatenation of the env observation and the one-hot 
                encoding of the option.
        - reset(self):
            - Resets the RM environment (as usual).
        - step(self, action):
            - Executes action in the RM environment as usual, but saves the relevant 
                information to compute the experience that will update the option
                policies.
        - did_option_terminate(self, option_id):
            - Returns True if the last action caused "option" to terminate.
        - get_experience(self):
            - Returns the off-policy experience necessary to update all the option
                policies.
    """
    ...
    def __init__(
        ...
        if use_self_loops:
            ...
            self.options = [
                (rm_id, u1, u2)
                for rm_id, rm in enumerate(env.reward_machines)
                for u1 in rm.delta_u
        ...
        self.option_observation_dict = spaces.Dict(
            {
                "features": env_obs_space,
                "option": spaces.Box(low=0, high=1, shape=(self.num_options,), dtype=np.uint8)
            }
        )
        ...
        self.option_observation_space = spaces.Box(
    ...
    def reset(self):
        """use this set to compute RM states that are reachable by the last experience
        (None means that all of them are reachable!)
        """
    ...
    def step(self, action):
        ...
        # exeucting the acton in the environment
        rm_obs, rm_rew, done, info = self.env.step(action)
    ...
    def did_option_terminate(self, option_id):
        # Note: options terminate when the current experience 
        # changes the RM state
    ...
    def _get_option_experience(
        ...
        opt_rew = rm_rew
        if u1 != u2 == un:
            # Extra positive reward because the agent accomplished this option
            opt_rew += self.r_max
        elif done:
            # Extra negative reward because the agent failed to accomplished this option
            opt_rew += self.r_max
    ...
    def get_experience(self):
        ...
        for option_id in range(self.num_options):
            ...
            if self.valid_states is None or (rm_id, u1) in self.valid_states:
                # Only add experiences that are possible (i.e., it is possible
                # to reach state u1 given the previous experience)
                exp = self._get_option_experience(option_id, obs, action, next_obs,
