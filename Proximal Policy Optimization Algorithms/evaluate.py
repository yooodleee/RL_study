...
from ray.rllib.utils.metrics import ENV_RUNNER_RESULTS, EPISODE_LEN_MEAN, EPISODE_RETURN_MEAN
...
class RolloutSaver:
    ...
    Currently supports two behavirours: the original, which
    ...
    is sorted with a key based on the episode number (0-indexed),
...
def run(
    ...
    if checkpoint:
        ...
        config_path=os.path.join(config_dir, "param.pkl")
    ...
    # If no pkl file found, require command line '--config'.
        ...
        if checkpoint:
            raise ValueError(
                "Could not fine params.pkl in either the checkpoint dir or "
    ...
    # Merge with 'evaluation_config' (first try from command line, than from pkl file).
    evaluation_config=copy.deepcopy(config_args.get("evaluation_config", {}))
    ...
    # Make no sence anyways, to have it set to None as we don't call
    ...
    # Create state from checkpoint, if provided.
...
class DefaultMapping(collections.defaultdict):
    ...
    def __missing__(self, key):
        self[key]=value=self.default_factory(key)   # single expression(variable_num = value_num)
        return value
...
def keep_going(steps: int, num_steps: int, episodes: int, num_episodes: int)-> bool:
    ...
    episode_limit_reached=num_episodes and episodes >= num_episodes # short circuit evaluation
    step_limit_reached=num_steps and steps >= num_steps # short circuit evaluation
...
def rollout(
    ...
    if hasattr(agent, "eval_env_runner_group") and isinstance(agent.eval_env_runner_group, EnvRunnerGroup):
        steps=0
        episodes=0
    ...
    # Agent has neither evaluation-nor rollout workers.
        ...
        if envs.registery.env_specs.get(agent.config["env"]):
            ...
            for agent_id, a_obs in multi_obs.items():
                if a_obs is not None:
                    ...
                    if p_use_lstm:
                        ...
                        a_action=agent_id.compute_single_action(
                            a_obs,
                            prev_action=prev_actions[agent_id],
                            prev_reward=prev_rewards[agent_id],
                            policy_id=policy_id,
