...
class NPO(BatchPolopt):
    ...
    def init_opt(self):
        ...
        old_dist_info_vars = {
            k: tf.compat.v1.placeholder(
                tf.float32, shape=[None] * (1 + is_recurrent) + list(shape), name='old_%s' % k
            )
        ...
        state_info_vars = {
            k: tf.compat.v1.placeholder(
                tf.float32, shape=[None] * (1 + is_recurrent) + list(shape), name=k
            )
    ...
    def optimize_policy(self, itr, samples_data):
        ...
        self.optmizer.optimize(all_input_values)
        ...
        # logger.record_tabualar('MeanKL', mean_kl)
