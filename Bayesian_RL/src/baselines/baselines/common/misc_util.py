...
def get_wrapper_by_name(env, classname):
    """
    Given an a gym environment possibly wrapped multiple times,
        returns a wrapper of classes named classname or raises ValueError
        if no such wrapper was applied.
    ...
    while True:
        if classname == currentenv.class_name():
            ...
            raise ValueError(
                "Couldn't find wrapper named %s" % classname
            )


def relatively_safe_pickle_dump(
        obj, path, compression=False
):
    """
    This is just like regular pickle dump, except from the fact that failure cases
        are different:

        - It's never possible end up with a pickle in corrupted state.
        - If a there was a different file at the path, that file will remain
            unchanged in the even of failure (provided that filesystem rename
            is atomic).
        - it is sometimes possible end up with useless temp file which needs to be
            deleted manually (it will be removed automatically on the next function
            call).

    The indended use case is periodic checkpoints of experiment state, such never
...
def pickle_load(path, compression=False):
    ...
    path: str
        path to the output file.
    compression: bool
        if true assumes that pickle was compreesed when created and
    ...
    if compression:
        with zipfile.ZipFile(
            path, "r", compression=zipfile.ZIP_DEFLATED
        ) as myzip:
