...
class REplayMemory(object):
    def __init__(self, max_size, state_shape, history_len, dtype='unit8'):
        """
        Args:
        ...
        logger.info("Creating experience replay buffer of {:1.f} GB ..."
                    "use a smaller buffer if you don't have enough CPU memory".format(np.prod(all_state_shape) / 1024.0**3))
        ...
        self.writer_lock=threading.Lock()   # a lock to guard writing to the moemory
...
class EnvRunner(object):
    ...
    def __init__(self, player, predictor, memory, history_len):
        ...
        """
        self.player=player
        self.num_actions=player.action_space.n  # 가능한 행동의 총 개수
        self.predictor=predictor
        self.memory=memory
        self.state_shape=memory.state_shape
        self.dtype=memory.dtype
        self.histoty=history_len
    ...
    def step(self, exploration):
        ...
        if self.rng.rand() <= exploration:
            ...
            q_values=self.predictor(history)[0][0]  # this is the bottlenack
        ...
        if isOver:
            ...
            if flush_experience:
                ...
                # Ensure that the wole episode of experience is continous in the replay buffer
    ...
    def recent_state(self):
        ...
        """
        expected_len=self.histoty_len - 1
...
class EnvRunnerManager(object):
    ...
    def _create_simulator_thread(self, idx):
        ...
        th.name="SumulatorThread-{}".format(idx)
        return th
...
class ExReplay(DataFlow, Callback):
    """
    ...
    * Produces batched samples by sampling the replay buffer. After producing
    ...
    def __init__(
        ...
        for k, v in locals().items():
            if k != 'self':
                setattr(self, k, v)
        self.exploration=1.0    # default initial dexploration
    ...
    def _init_memory(self):
        ...
        with get_tqdm(total=self._init_memory_flag) as pbar:
