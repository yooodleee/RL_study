...
def inference(args):
    ...
    with tf.device("/cpu:0"):
        ...
        if 'distance' in info:
            ...
            np.save(outdir + '/distances.py', mario_distances)
        env.close()
...
def main(_):
    ...
    parser.add_argument('--recordSignal', action='store_true', help="Record images of true processedd input to network")
    parser.add_argument('--render', action='store_true', help="Render the gym environment video online")
    parser.add_argument('--envWrap', action='store_true', help="Process input in env_wrapper (no change in input size or network)")
    ...
    parser.add_argument('--noop', action='store_run', help="Add 30-noop for inference too (recommended by Nature paper, don't know?)")
    parser.add_argument('--acRepeat', type=int, default=0, help="Actions to be repeated at inference. 0 means default. applies if envWrap is True.")
