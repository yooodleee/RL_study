...
class Push(gym.Env):
    ...
    def __init__(self, mode="default"):
        if mode == "default":
            ...
            raise ValueError(
                "Mode must be default. Implement some more variants if you want to experiment!"
            )
        ...
        # channels are 0: agent, 1: goals, boxes, 3: obstacles, 4: time remaining
    ...
    def _reset(self):
        ...
        # initialize time-remaining to 1.
        ...
        locs = np.random.choice(
            (self.w - 2 - (2 * self.walls)) ** 2, 1 + self.n_goals + self.n_boxes + self.n_obstacles, replace=False
        )
    ...
    def print(self):
        ...
        print(pretty)
        pretty("TIMELEFT", self.state[0, 0, 3 + self.use_obst])
    ...
    def restore_full_state(self, state_dict):
        self.___dict__.update(state_dict)
...
if __name__ == "__main__":
    """If called dir with arg "random", evaluates the average return of a random policy.
    If called without args, starts an interactive game plated with wasd to move, 1 to quit.
    """
    env = Push()
