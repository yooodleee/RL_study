...
from src.utils.pytorch_utils import View, nn_init, logsumexp
from src.utils.einsum import einsum
from src.models.transitions import build_transition_fn, MLPRewardFn
from src.models.encoding import atari_encoder, push_encoder
...
class TreeQNPolicy(nn.Module):
    ...
    def __init__(
        ...
        if transition_fun_name == "two_layer":
            # going to introduce residuals manually inside the transition function, so turning this off...
        ...
        if self.transition_fun_name == "two_layer":
            self.transition_fun1, self.transition_fun2 = build_transition_fn(transition_fun_name,
                                                                               embedding_dim,
                                                                               nonlin=self.transition_nonlin,
                                                                               num_actions=self.num_actions)
        else:
            self.transition_fun = build_transition_fn(transition_fun_name, 
                                                      embedding_dim, 
                                                      nonlin=self.transition_nonlin,
                                                        num_actions=self.num_actions)
    ...
    def forward(self, ob, volatile=False):
        ...
        param
        ----------
            ob: [batch_size x channels x height x width]

        return
        -----------
            [batch_size x num_actions], --Q-values
    ...
    def obs_to_variable(self, obs, volatile=False):
    ...
    def sample(self, Q):
        if self.use_actor_critic:
            ...
            a = torch.multinomial(pi, 1).square()
    ...
    def tree_planning(
        ...
        ----------
            dict tree_result:
                - "embeddings": list of length tree_depth, [batch_size * num_actions ^ depth x embedding_dim]
                    state representations after tree planning
                - "values": list of length tree depth, [batch_size * num_actions ^ depth x 1]
                    values predicted form each embedding
                - "rewards": list of length tree_depth, [batch_size * num_actions ^ depth x 1] 
                    rewards predicted from each transition
        """

        tree_result = {"embeddings": [x], "values": []}
        if self.predict_rewards:
            tree_result["rewards"] = []
    ...
    def planning(self, x):
        ...
        param
        -----------
            x: [batch_size x embedding_dim] state representations

        returns
        ------------
            - [batch_size x embedding_dim x num_actions] state-action values
            - [batch_size x num_actions x embedding_dim] state representations after
                planning one step used for regularizing/grounding the transition model.
        """
        batch_size = x.size(0)
        ...
        for i in range(1, self.tree_depth + 1):
            ...
            if i < self.tree_depth:
                ...
                if self.value_aggregation == "max":
                    ...
                    raise ValueError(
                        "Unknown value aggregation function %s" % self.value_aggregation
                    )
                backup_values = (
                    (1 - self.td_lambda) 
                    * tree_result["values"][-i - 1]
                    + (self.td_lambda)
                    * max_backup.view(-1, 1)
                )
            else:
...
class DQNPolicy(TreeQNPolicy):
    ...
    def __init__(
        ...
        # Just to make sure that calculating num of params is correct
        ...
        if extra_layers > 0:
            self.transition_fun = nn.init.xavier_normal(
                nn.Parameter(torch.Tensor(self.embedding_dim, self.embedding_dim))
            )
    

    def planning(self, x):
        tree_result = {"embeddings": [x]}
