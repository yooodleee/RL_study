...
class _Batcher(object):
    """
    A train layer around the Batcher TensorFlow operations.

    It shares some of the interface with queues (close(), name) 
        to be able to use it correctly as the input to a QueueRunner.
    """
    ...
    def __init__(self,
        ...
        self._handle = batcher_ops.batcher(minimum_batch_size,
                                            maximum_batch_size,
                                            timeout_ms or -1)
    ...
    def set_outputs(self, flat_result, computation_id):
        return batcher_ops.batcher_set_outputs(self._handle, 
                                               flat_result, 
                                               computation_id)
    
    def compute(self, flat_args, output_dtypes):
        return batcher_ops.batcher_compute(self._handle, 
                                           flat_args, 
                                           output_dtypes)
    
    def close(self, cancle_pending_enqueues=False, name=None):
        del cancle_pending_enqueues
...
def batch_fn_with_options(minimum_batch_size=1,
    ...
    When the decorated function is called, it creates an operation 
        that adds the inputs to a queue, waits until the computation 
        is done, and returns the tensors.

    The inputs must be nests (see 'tf.contrib.framework.nest') and the first 
        dimensions of each tensor in the nest must have size 1.
    ...
    Example usage:
        ...
        output0 = fn(tf.constant([1]), tf.constant([2]))  # will be batched 
                    with the next call.
        
        output1 = fn(tf.constant([3]), tf.constant([4])
    
    Note, gradients are currently not supported.
    Note, if minimum_batch_size == maximum_batch_size and timeout_ms=None, 
        then the batch size of input arguments will be set statically. 
    Otherwise, it will be None.

    Args:
        minimum_batch_size: The minimum batch size before processing starts.
        maximum_batch_size: The maximum batch size.
        timeout_ms: Milliseconds after a batch of samples is requested 
            before it is processed, even if the batch size is smaller than 
            'minimum_batch_size'. If None, there is no timeout.
    ...
    def decorator(f):
        ...
        def wrapper(*args):
            ...
            flat_args = [tf.convert_to_tensor(arg) \
                         for arg in nest.flatten(args)]

            if batcher[0] is None:
                # Remove control dependencies which is necessary 
                # when created in loops, etc.
                with tf.control_dependencies(None):
                    ...
                    if minimum_batch_size == maximum_batch_size and \
                        timeout_ms is None:
                    ...
                    inputs, computation_id = \
                        batcher[0].get_inputs(input_dtypes)
                    nest.map_structure(
                        lambda i, a: i.set_shape([batch_size] \
                                                 + a.shape.as_list()[1:]),
                        inputs, flat_args)
                    ...
                    tf.compat.v1.train.add_queue_runner(
                        tf.compat.v1.train.QueueRunner(batcher[0], [set_op]))
