...
class _Batcher(object):
    ...
    def set_outputs(self, flat_result, computation_id):
        return batcher_ops.batcher_set_outputs(self._handle, flat_result, computation_id)
...
def batch_fn_with_options(
    ...
    inputs to a queue, waits until the computation is done, and returns the tensors.
    The inputs must be nests (see 'tf.contrib.framework.nest') and the first dimensions
    of each tensor in the nest must have size 1.
    ...
    the batch size of input arguments will be set statically. Otherwise, it will be None.
