...
LEVEL_MAPPING = collections.OrderedDict([
                ('rooms_collec_good_objects_train', 
                 'room_collect_good_objects_test'),
                ('room_exploit_deferred_effects_train', 
...
HUMAN_SCORES = {'rooms_collect_good_objects_test': 10,
                'rooms_exploit_deferred_effects_test': 85.65,
                'rooms_select_nomatching_object': 65.9,
                'rooms_watermaze': 54,
...
RANDOM_SCORES = {'rooms_collect_good_objects_test': 0.073,
                'rooms_exploit_deferred_effects_test': 8.501,
                'rooms_select_nomatching_object': 0.312,
                ...
                'explore_goals_locations_small': 7.66,
                'explore_goals_locations_large': 3.14,
                'explore_object_rewards_few': 2.073,
                'explore_object_rewards_many': 2.438}

ALL_LEVELS = frozenset(['rooms_collect_good_objects_train',
                        'rooms_collect_good_objects_test',
                        'rooms_exploit_deferred_effects_train',
                        'rooms_exploit_deferred_effects_test',
                        'rooms_select_nomatching_object',
                        'rooms_watermaze',
...
def _transform_level_returns(level_returns):
    ...
    for level_name, returns in new_level_returns.iteritems():
        if level_name in test_set:
            if not returns:
                raise ValueError('Missing returns for level: \'%s\': ' \
                                 % level_name)
        else:
            tf.compat.v1.logging.info('Skipping level %s calculation.', \
                                      level_name)
...
def compute_human_normalized_score(level_returns, per_level_cap):
    ...
    Levels that have different training and test versions, will 
        use the returns
    for the training level to calculate the score. E.g.,
        'rooms_collect_good_objects_train' will be used for 
        'rooms_collect_good_objects_test'. 
    All returns for levels not in DmLab-30 will be ignored.
    ...
    Raises:
        valueError: If a level is missing from 'level_returns' or 
            has no returns.
    """
    ...
    def human_normalized_score(level_name, returns):
        ...
        if per_level_cap is not None:
            human_normalized_score = min(human_normalized_score, 
                                         per_level_cap)
