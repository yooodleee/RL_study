...
learning_rate=5e-4  # first trial: 5e-4, second trial: 1e-4, third trial: 1e-3, forth trial: 5e-5, fifth frial: 5e-5 gamma: 0.90, sixth trial: 1e-4 gamma: 0.90, seventh trial: 5e-4 gamma: 0.90
...
# vf_coef is the value function coefficient, If we set it to 0.5, then the value function liss will be half the policy loss
vf_coef=0.5
...
# the variance of the advantage is reduced, this is done to improve the training process because the advantage is used to calculate the policy lss
...
# _init_setup_model is a boolean that indicates if the model will be initialized after being created, set to True to initilize the model
