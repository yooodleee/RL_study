'''
Code for testing the trained PPO agent in the Breakout environment 
    using stable baseline 3's PPO
...
Note: For understanding the code, it is recommended to read the documentation 
    of stalbe baselines 3's PPO implementation and to understand 
    the parameters of the model, go to config.py and read the comments 
    of each parameters.
'''
...
if config.log_to_wandb:
    wandb.init(project=config.project_test, 
...
if config.pretrained:
    # Unzip the file PPO_Breakout_1M.zip and store the unzipped files 
    # in the folder PPO_Breakout_unzipped
    unzip_file(config.saved_model_path, config.unzip_file_path)

# We start with a single environment for Breakout 
# with render mode set to human
...
# Finally, we wrap the environment with the VecFrameStack wrapper 
# which stacks the observations over the last 4 frames
env = VecFrameStack(env, n_stack=config.n_stack)  # Output shape: (4, 84, 84)

# Create the model
model = PPO(policy=config.policy, env=env, verbose=config.verbose)

# Load the model if config.pretrained is set to True in config.py
if config.pretrained:
    # Load the model components, 
    # including the policy network and the value network
    model.policy.\
...
for episode in range(config.test_episodes):
    # Reset the environment and stack the inital state 4 times
    ...
    episode_reward = 0
    while not done:
        # Take an action in the environment accordint to the policy 
