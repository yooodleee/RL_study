'''
Code for testing the trained PPO agent in the Breakout environment using stable baseline 3's PPO
implementation and logging the results to wandb.

Note: For understanding the code, it is recommended to read the documentation of stalbe baselines 3's PPO implementation and to
understand the parameters of the model, go to config.py and read the comments of each parameters.
'''
...
if config.log_to_wandb:
    wandb.init(project=config.project_test, entity=config.entity, sync_tensorboard=config.sync_tensorboard, name=config.name_test, notes=config.notes)
...
for episode in range(config.test_episodes):
    ...
    episode_reward=0
    while not done:
        # Take an action in the environment accordint to the policy of the trained agent
