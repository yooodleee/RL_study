...
learning_rate=5e-4  # first_trail: 5e-4, seconde_trail: 1e-4, third trail: 1e-3, fourth_trail: 5e-5, fifth_trail: 5e-5 gamma: 0.90, sixth_trail: 1e-4 gamma: 0.90, seventh_trail: 5e-4 gamma: 0.90
...
# batch_size is the number of samples that will be taken from the reply buffer for training the model
batch_size=64

# tau is the soft update cofficient for updating the target network if set to 1 then the target network is hard updated every target_update_interval timesteps
tau=1.0
...
# Take into account that the number of timesteps is not the number of episodes, in a game loke breakout, the agent takes an action every frame,
