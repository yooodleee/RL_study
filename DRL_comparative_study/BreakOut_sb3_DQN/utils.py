...
It makes sure that the rewards of each episode are stored in a list and that the current spisode reward is reset
'''
...
'''
The CusomWandbCallback is a callback* that logs the mean reward of the last 100 episodes to wandb.
...
class CustomWandbCallback(BaseCallback):
    ...
    def _on_step(self)-> bool:
        ...
        if self.n_calls % self.check_freq == 0:
            # Gather rewards from all environments, by all environents we mean all the vectorized environment, usually there is only 1 environment in the vectorized environment
            all_rewards=[]
            ...
            # If there are rewards in all_rewards, compute the mean rewards of the last 100 episodes and log it to wnadb
            if all_rewards:
...
def make_env(env_id, seed=0):
    ...
    def _init():
        ...
        env.seed(seed)
        # Wrap the environment with the AtariWrapper which wrpas the environment correctly
