...
It makes sure that the rewards of each episode are stored in a list and that 
    the current spisode reward is reset
'''
class RewardLogger(gym.Wrapper):
    ...
    # The reset function is called every time the environment 
    # is reset (at the beginning of each episode)
...
'''
The CusomWandbCallback is a callback* that logs the mean reward 
    of the last 100 episodes to wandb.
A callback is a function that is called at the end of each episode 
    to perform some action, in this case, the action is logging 
    the mean reward of the last 100 episodes to wandb.
'''
class CustomWandbCallback(BaseCallback):
    def __init__(self, check_freq, save_path, verbose=1):
        super(CustomWandbCallback, self).__init__(verbose)
        # Define the frequnecy at which the callback is called
    ...
    def _on_step(self) -> bool:
        ...
        It returns True if the callback should be called again, and 
            False otherwise.
        To do this, it checks if the number of calls to the callback is 
            a multiple of the check_freq.
        If it is, it computes the mean reward of the last 100 episodes 
            and logs it to wandb.
        It also saves the model if the mean reward is greater than 
            the best mean reward.
        '''
        # Check if the number of calls to the callback is a multiple 
        # of the check_freq
        if self.n_calls % self.check_freq == 0:
            # Gather rewards from all environments, 
            # by all environents we mean all the vectorized environment, 
            ...
            for env in self.training_env.envs:  
                ...
                if isinstance(logger_env, RewardLogger):
                    # If it is, get the rewards of each episode 
                    # and store them in all_rewards

                    # extend is used to add the elements of a list 
                    # to another list
                    all_rewards.extend(logger_env.get_episode_rewards())    

            # If there are rewards in all_rewards, compute the mean rewards 
            # of the last 100 episodes and log it to wnadb
            if all_rewards:
                # Compute the mean reward of the last 100 episodes
                mean_reward = np.mean(all_rewards[-self.check_freq:])
                # Log the mean reward of the last 100 episods to wandb
...
def make_env(env_id, seed=0):
    ...
    def _init():
        ...
        # this is done to make the results reproducible
        env.seed(seed)
        # Wrap the environment with the AtariWrapper 
        # which wrpas the environment correctly
        env = AtariWrapper(env)
        # Wrap the environment with the RewardLogger wrapper 
        # which logs the rewards of each episode to wandb
