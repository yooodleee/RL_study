'''
Code for testing the trained A2C agent in the Breakout environment using 
    stable baselines 3's A2C
...
understand the parameters of the model, go to config.py and read the comments 
    of each parameter.
'''
...
if config.pretrained:
    # Unzip the file a2c_Breaout_1M.zip and store the unzipped files in the 
    # folder a2c_Breakout_unzipped
    unzip_file(config.saved_model_path, config.unzip_file_path)

# We start with a single environment for Breakout with render mode 
# set to human
env=make_env("BreakoutNoFrameskip-v4")
# We then wrap the environment with the DummyVecEnv wrapper which converts the 
# environment to a single vectorized environment
env=DummyVecEnv([env])  # Output shape: (1, 84, 84)
# Finally, we wrap the environment with the VecFrameStack wrapper which stacks 
# the observations over the last 4 frames
env=VecFrameStack(env, n_stack=config.n_stack)  # Output shape: (4, 84, 84)

# Create the model
model=A2C(policy=config.policy, env=env, verbose=config.verbose)

# Load the model if config.pretrained is set to True in config.py
if config.pretrained:
    # Load the model components, including the policy network and the 
    # values network
    model.policy.load_state_dict(torch.load(os.pardir.join(config.\
                                                           unzip_file_path, 
                                                           "policy.pth")))
...
for episode in range(config.test_episodes):
    ...
    episode_reward=0
    while not done:
        # Take an action in the environment according to the policy 
