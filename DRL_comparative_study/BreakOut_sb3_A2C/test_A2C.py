...
if config.pretrained:
    # Unzip the file a2c_Breaout_1M.zip and store the unzipped files in the folder a2c_Breakout_unzipped
...
if config.pretrained:
    # Load the model components, including the policy network and the values network
    model.policy.load_state_dict(torch.load(os.pardir.join(config.unzip_file_path, "policy.pth")))
