...
# check_freq is the frequency at which the callback is called, the callback is called every 2000 timesteps
check_freq=2000
...
'''
Hyperparameters of the model {learning_rage, gamma, device, n_steps, gae_lambda, ent_coaf, vf_coef, max_grad_norm, rms_prop_eps, use_rms_prop, use_code, sde_sample_freq, normalize_advantage}
'''
...
learning_rate=5e-4  #first trial: 5e-4 #second trial: 1e-4 #third trial: 1e-3 #forth trial: 5e-5 #fifth trial: 5e-5 gamma=0.90 #sixth trial: 1e-4 gamma=0.90 #seventh trial: 5e-4 gamma=0.90
...
# vf_grad_norm is the value function coefficient, If we set it to 0.5, then the value function loss will be half the policy loss
vf_coef=0.5
...
# sde_sample_freq is the frequency at which the noise added to the actions. If set to -1, the noise will be added every timestep
sde_sample_freq = -1

# normalize_advantage is a boolean that indicates if the advantage will be normalized, by normalizing the advantage,
# the variance of the advantage is reduced, this is done to improve the training procces because the advantage is used to calculate the policy loss
normalize_advantage=False

# stats_window_size is the size of the window used to calculated the mean and standard deviation of the advantage
...
# total_time_step is the total number of timesteps that the model will be trained. In this case, the model will be trained for 1e7 timesteps
...
notes="a2c_breakout with parameters: {}".format(locals())   # locals() returns a dictionary will the local variables, in this case, all the varialbes in this file
