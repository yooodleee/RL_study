...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        if self.policy_type == "Gaussian":
            ...
            if self.automatic_entropy_tuning is True:
                self.target_entropy = \
                    -torch.prod(torch.Tensor(action_space.shape).\
                                to(self.device)).item()
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        with torch.no_grad():
            next_state_action, \
                next_state_log_pi, _ = self.policy.sample(next_state_batch)
            qf1_next_target, \
                qf2_next_target = self.critic_target(next_state_batch, 
                                                     next_state_action)
    ...
    def save_checkpoint(self, env_name, suffix="", ckpt_path=None):
        ...
        if ckpt_path is None:
            ckpt_path="checkpoint/sac_checkpoint_{}_{}".format(env_name, 
                                                                suffix)
        ...
        torch.save({"policy_state_dict":self.policy.state_dict(),
                    ...
                    "policy_optimizer_state_dict":\
                        self.policy_optim.state()}, ckpt_path)
