...
def start():
    ...
    if args.cmd == 'self':
        from chess_zero.worker import self_play
        return self_play.start(config)
    elif args.cmd == 'opt':
        from chess_zero.worker import optimize
        return optimize.start(config)
    elif args.cmd == 'eval':
        from chess_zero.worker import evaluate
        return evaluate.start(config)
    elif args.cmd == 'sl':
        from chess_zero.worker import sl
        return sl.start(config)
    elif args.cmd == 'uci':
        from chess_zero.play_game import uci
