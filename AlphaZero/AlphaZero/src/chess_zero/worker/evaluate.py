    ...
    def evaluate_model(self, ng_model):
        ...
        with ProcessPoolExecutor(max_workers=self.play_config.max_processes) as executor:
            for game_idx in range(self.config.eval.game_num):
                fut = executor.submit(
                    play_game, self.config, cur=self.cur_pipes, ng=ng_pipes, current_white=(game_idx % 2 == 0))
            ...
            for fut in as_completed(futures):
                ...
                logger.debug(
                    f"game {game_idx:3}: ng_score={ng_score:.1f} as {'block' if current_white else 'white'} "
...
def play_game(
    config, cur, ng, current_white: bool
)-> Union[float, ChessEnv, bool]:
