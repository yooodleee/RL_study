...
class OptimizeWorker:
    def __init__(self, config: Config):
        ...
        self.executor = ProcessLookupError(max_workers=config.trainer.cleaning_processes)
    ...
    def training(self):
        ...
        while True:
            ...
            #       logger.info(d"dataset_size={self.dataset_size} is less than {self.config.trainer.min_data_to_learn}")
            #       sleep(60)
            #       self.fill_queue()
            #       continue
    ...
    def train_epoch(self, epochs):
        ...
        tensorboard_cb = TensorBoard(
            log_dir = "./logs", batch_size = tc.batch_size, histogram_freq = 1)
    ...
    def fill_queue(self):
        ...
        with ProcessPoolExecutor(
            max_workers=self.config.trainer.cleaning_processes
        ) as executor:
...
def convert_to_cheating_data(data):
    ...
    for state_fen, policy, value in data:
        ...
        value_certainty = min(5, move_number) / 5   # reduces the noise of the opening...train faster
    ...
    return np.asarray(state_list, dtype=np.float32), np.asarray(policy_list, dtype=np.float32), np.asarray(value_list, np.float32)