...
class Logger:
    ...
    def _log_params(self):
        with summary("Logs/" + self.log_dir) as writer:
    ...
    def log(self, *args):
        if not self._turn_on:
            print(
                "First you should turn the logger on once, "
                "via on() method to be able to log parameters"
            )
            return 
        self._off()
        ...
        assert self.to_gb(ram.used) < 0.98 * self.to_gb(ram.total), "RAM usage exceed permitted limit!"
        ...
        torch.save(
            {
                ...
                "value_network_state-dict": self.agent.value_network.state_dict(),
                "discriminator_state_dict": self.agent.discriminator.state_dict(),
                "q_value1_opt_state_dict": self.agent.q_value1_opt.state_dict(),
                "q_value2_opt_state_dict": self.agent.q_value2_opt.state.dict(),
