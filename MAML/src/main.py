...
flags.DEFINE_integer('meta_lr', 0.001, 'the base learning rate of the generator')
flags.DEFINE_integer('update_batch_size', 5, 'number of examples used for inner gradient update (K for K-shot learning).')
flags.DEFINE_integer('update_lr', 1e-3, 'step size alpha for inner gradient update.') # 0.1 for omniglot
...
## Logging saving, and testing ops
flags.DEFINE_bool('log', True, 'if True, do not log summaries, for debugging code.')
flags.DEFINE_string('logdir', '/tmp/data', 'directory for summaries and checkpoints.')
flags.DEFINE_bool('resume', True, 'resume training if there is a model available')
flags.DEFINE_bool('train', True, 'True to train, False to test.')
flags.DEFINE_integer('test_iter', -1, 'iteration to load model (-1 for latest model)')
flags.DEFINE_bool('test_set', False, 'Set to true to test on the test set, False for the validation set.')
...
def train(
    ...
    num_classes = data_generator.num_classes    # for classification, 1 othwerwise
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr != 0) and itr % SAVE_INTERNAL == 0:
...
def test(
    ...
    for _ in range(NUM_TEST_POINTS):
        if 'generate' not in dir(data_generator):
            ...
            if FLAGS.baseline == 'oracle':  # NOTE -> this flag is specific to sinusoid
                batch_x = np.concatenate([batch_x, np.zeros([batch_x.shape[0], batch_x.shape[1]])], 2)
                batch_x[0, :, 1] = amp[0]
                batch_x[0, :, 2] = phase[0]
    ...
    out_filename = FLAGS.logdir \
                 + '/' + exp_string \
                 + '/' + 'test_ubs' + str(FLAGS.update_batch_size) + '_stepsize' \
                 + str(FLAGS.update_lr) + '.csv'
    
    out_pkl = FLAGS.logdir + '/' \
            + exp_string + '/' \
            + 'test_ubs' + str(FLAGS.update_batch_size) + '_stepsize' \
            + str(FLAGS.update_lr) + '.pkl'
    
    with open(out_pkl, 'wb') as f:
        pickle.dump({'mess': metaval_accuracies}, f)
...
def main():
    ...
    if FLAGS.baseline == 'oracle':
        ...
        dim_input = 3
        FLAGS.pretrain_iteration += FLAGS.metatrain_iterations
    ...
    if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'omniglot':
        ...
        if FLAGS.train: # only construct training model if needed
            ...
            inpute_tensors = {'inputa': inputa, 'inputb': inputb, 'labela': labela, 'labelb': labelb}
    ...
    if FLAGS.train or not tf_data_load:
        model.construct_model(inpute_tensors=inpute_tensors, prefix='metatrain_')
    if tf_data_load:
        model.construct_model(inpute_tensors=metaval_input_tensors, prefix='metaval_')
    model.summ_op = tf.compat.v1.summary.merge_all()

    saver = loader = tf.compat.v1.train.Saver(
    ...
    exp_string = 'cls_' + str(FLAGS.num_classes) \
               ...
               + '.ubs_' + str(FLAGS.train_update_batch) \
