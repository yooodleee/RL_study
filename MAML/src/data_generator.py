...
class DataGenerator(object):
    ...
    def make_data_tensor(self, train=True):
        ...
        filename_queue = tf.compat.v1.train.string_input_producer(tf.convert_to_tensor(all_filenames), shuffle=False)
        print('Generate image processing ops')
        ...
        for i in range(self.batch_size):
            ...
            if FLAGS.datasource == 'omniglot':
                ...
                # get rotation class (e.g. 0,1,2,0,0 if there are 5 classes)
                rotations = tf.compat.v1.multinomial(tf.compat.v1.log([[1., 1., 1., 1.]]), self.num_classes)
            
            lable_batch = tf.convert_to_tensor(labels)
            new_list, new_label_list = [], []
            for k in range(self.num_samples_per_class):
                class_idxs = tf.range(0, self.num_classes)
                class_idxs = tf.compat.v1.random_shuffle(class_idxs)
                ...
                new_label_list.append(tf.gather(lable_batch, true_idxs))
    ...
    def generate_sinusoid_batch(self, train=True, input_idx=None):
        ...
        input_idx is used during qualitative testing --the num of examples 
