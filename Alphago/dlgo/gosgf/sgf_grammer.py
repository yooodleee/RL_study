...
def make_tree(
    ...
    game_tree       -- Coarse_game_tree
    root            -- node
    node_builder    -- function taking paramteres (parent node, property map)
                        returning  a node
    node_adder      -- function taking a pair (parent node, child node)

    Buildes a tree of nodes corresponding to this GameTree, calling
        node_builder() to make new nodes and node_adder() to add child nodes
        to their parent.
    ...
    while to_build:
        ...
        if index < len(game_tree.sequence) - 1:
            child = node_builder(
                node, game_tree.sequence[index + 1]
            )
            node_adder(node, child)
            to_build.append(
                (
                    child, game_tree, index + 1
                )
            )
        else:
            node._children = []
