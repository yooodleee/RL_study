...
class Sampler:
    # sample training and test data from zipped sgf files
    # such that test data is kept stable
    def __init__(
            self,
            data_dir='data',
            num_test_games=100,
        ...
        self.test_games = []
        self.train_games = []
        self.test_folder = 'test_sampler.py'
        self.cap_year = cap_year

        random.seed(seed)
        self.compute_test_sampler()
    ...
    def draw_data(
        ...
        if data_type == 'test':
            ...
            raise ValueError(
                data_type + " is not a valid data type, choose from 'train' or 'test'"
            )
    ...
    def compute_test_samples(self):
        # if not already existing, create local file to store
        # fixed set of test samples
        if not os.path.isfile(self.test_folders):
            test_games = self.draw_samples(self.num_test_games)
            test_sample_file = open(self.test_folder)
