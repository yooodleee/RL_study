...
class KGSIndex:
    def __init__(
            self,
            kgs_url='http://u-go.ney/gamerecords/',
    ...
    def load_index(self):
        # create the actual index representation from the previously
        # downloaded or cached html
        index_contents = self.create_index_page()
        split_page = [
            item for item in index_contents.split('<a href="')
        ...
        for url in self.urls:
            ...
            num_games = int(
                split_file_name[len(split_file_name) - 2]
            )
