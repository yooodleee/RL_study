...
class LocalGtpBot:
    ...
    def opponent_cmd(opponent):
        if opponent == 'gnugo':
            ...
            raise ValueError(
                'Unknown bot name {}'.format(opponent)
            )
    ...
    def play_our_move(self):
        ...
        if move.is_pass:
            ...
            self.command_and_response(
                'play {} {}\n'.format(our_name, pos)
            )
            sgf_move = self.sgf.coordinames(move)
        self.sgf.append(
    ...
    def play_their_move(self):
        ...
        if pos.lower() == 'resign':
            ...
            self.game_state = self.game_state.apply_move(move)
            self.sgf.append(
                ';{}[{}]\n'.format(their_name, self.sgf.coordinates(move))
            )