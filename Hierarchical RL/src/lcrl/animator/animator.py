...
from lcrl.environments.SlipperGrid import SlipperyGrid
...
def animate(
    ...
    if isinstance(mdp, SlipperyGrid):
        ...
        print('---------------------------------\n')
        print('Creating a git for the trained policy: ')
        ...
        for i in tqdm.tqdm(range(len(executed_policy))):
            if i == 0:
                ...
                inits_y, inits_x = path_y[0], path_x[0]
                plt.scatter(
                    path_y,
                    path_x,
                    c='lime',
            ...
            plt.savefig(
                os.path.join(animation_dir, 'image_file_' + str(i) + '.png')
            )
        ...
        imageio.mimsave(
            os.path.join(animation_dir, 'executed_policy.gif'), images, fps=55
        )
    
    else:
        raise NotImplementedError(
            'The animator does not support this environment yet.'
        )