        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(args.num_episodes):
                ...
                while True:
                    # run policy
                    fetched = sess.run(
                        [probs, sample, vf, state_out_0, state_out_1],
                    ...
                    timestep_limit = env.spec.tags.get('wrapper_config.TimeLimig.max_episode_steps')
                    if timestep_limit is None: timestep_limit = env.spec.timestep_limit
                    if terminal or length >= timestep_limit:
                        if length >= timestep_limit or not env.metadata.get('sementics.autoreset'):
                            last_state = env.reset()
                        last_features = np.zeros((1, 256), np.float32); last_features = [last_features, last_features]
...
def main():
    ...
    parser.add_argument('--ckpt', default="../modles/doom/doom_ICM", help="checkpoint name")
