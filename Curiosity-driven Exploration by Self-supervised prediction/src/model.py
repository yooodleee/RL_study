...
class LSTMPolicy(object):
    def __init__(self, ob_space, ac_space, desingHead='universe'):
        self.x = x = tf.compat.v1.placeholder(tf.float32, [None] + list(ob_space), name='x')
        size = 256
        if desingHead == 'nips':
            x = nipsHead(x)
        elif desingHead == 'nature':
            x = natureHead(x)
        elif desingHead == 'doom':
            x = doomHead(x)
        elif 'title' in desingHead:
        ...
        lstm = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(size, state_is_tupe=True)
        ...
        c_in = tf.compat.v1.placeholder(tf.float32, [1, lstm.state_size.c], name='c_in')
        h_in = tf.compat.v1.placeholder(tf.float32, [1, lstm.state_size.h], name='h_in')
        self.state_in = [c_in, h_in]

        state_in = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c_in, h_in)
        lsmt_outputs, lstm_state = tf.compat.v1.nn.dynamic_rnn(
            lstm, x, initial_state=state_in, sequence_length=step_size, time_major=False
        )
        lstm_c, lstm_h = lstm_state
        x = tf.reshape(lsmt_outputs, [-1, size])
        self.vf = tf.reshape(linear(x, 1, "value", normalized_columns_initializer(1.0)), [-1])
        self.state_out = [lstm_c[:1, :], lstm_h[:1, :]]

        # [0, :] means pick action of first state from batch. Hardcoded b/c
        # batch=1 during rollouts collection. Its not used during batch training.
...
class StateActionPredictor(object):
    def __init__(self, ob_space, ac_space, designHead='universe'):
        ...
        # sample: 1-hot encoding of sampled action from policy: [None, ac_space]
        ...
        self.sample = asample = tf.compat.v1.placeholder(tf.float32, [None, ac_space])
        ...
        self.forwardloss = self.forwardloss * 288.0 # Factored out to make hyperparams not depend on it.
    ...
    def pred_bonus(self, s1, s2, asample):
        """
            returns bonus predicted by forward mode
                input: s1, s2: [h, w, ch], asample: [ac_space] 1-hot encodinf
                output: scalar bonus
        """
        sess = tf.compat.v1.get_default_session()
        ...
        error = sess.run(self.forwardloss, {self.s1: [s1], self.s2: [s2], self.asample: [asample]})
...
class StatePredictor(object):
    ...
    def __init__(self, ob_space, ac_space, designHead='universe', unsupType='state'):
        # input: s1, s2: : [None, h, w, ch] (usually ch=1 or 4)
        # asample: 1-hot encodinf of sampled action from policy: [None, ac_space]
        input_shape = [None] + list(ob_space)
        self.s1 = phi1 = tf.compat.v1.placeholder(tf.float32, input_shape)
        self.s2 = phi2 = tf.compat.v1.placeholder(tf.float32, input_shape)
        ...
        # feature encodinf: phi1: [None, LEN]
        if designHead == 'universe':
            phi1 = universeHead(phi1)
            if self.stateAenc:
                with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=True):
                    phi2_aenc = universeHead(phi2)
        elif 'title' in designHead: # for mario titles
            phi1 = universeHead(phi1, nConvs=2)
            if self.stateAenc:
                with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=True):
                    phi2_aenc = universeHead(phi2)
        else:
            print('Only universe desingHead implemented for state prediction baseline.')
            exit(1)
    ...
    def pred_state(self, s1, asample):
        """
            returns state predicted by forward mode
                input: s1: [h, w, ch], asample: [ac_space] 1-hot encodinf
                output: s2: [h, w, ch]
        """
        sess = tf.compat.v1.get_default_session()
