...
class MuZero:
    ...
    def load_model(self, checkpoint_path=None, replay_buffer_path=None):
        ...
        if replay_buffer_path:
            ...
            print(f"\nIntializing replay buffer with {replay_buffer_path}")
        else:
            ...
            while 0 < budget or any(running_experiments):
                for i, experiment in enumerate(running_experiments):
                    if experiment and experiment.config.training_steps <= ray.get(
                        ...
                        result = experiment.text(False, num_tests=num_tests)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        # Train directory with: Python muzero.py cartpole
        muzero = MuZero(sys.argv[1])
        muzero.train()
    elif len(sys.argv) == 3:
        # Train directory with : Python muzero.py cartpole '{"lr_init": 0.01}'
        ...
        while choice not in valid_inputs:
            choice = input("Invalid input, enter a number listed aboive: ")
        ...
        while True:
            ...
            if choice == 0:
                ...
                muzero.test(render=True, opponent="humman", muzero_player=0)
            elif choice == 5:
                env = muzero.Game()
                env.reset()
                env.render()
