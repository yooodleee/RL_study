...
def identity(x: torch.Tensor)-> torch.Tensor:
    ...
    """
    base.assert_dtype(
        x, (torch.float16, torch.float32, torch.float64)
    )
    return x
...
def power(x: torch.Tensor, p: float)-> torch.Tensor:
    ...
    return torch.sign(x) \
        * (torch.pow(torch.abs(x) / q + 1.0, p) - 1) / q
...
def transform_from_2hot(
    ...
    support_space = torch.linspace(
        min_value, max_value, num_bins
    )
    scalar = torch.sum(
        probs * torch.unsqueeze(support_space, 0), -1
    )
    return scalar