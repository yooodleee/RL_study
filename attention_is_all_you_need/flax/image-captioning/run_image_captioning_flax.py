"""
Flax-tuning the library vision-encoder-decoder models for image captioning.
"""
...
class ModelArguments:
    ...
    model_name_or_path: str= field(
        metadata={"help": "The model checkpoint for weights initializations."},
    )
    cache_dir: Optional[str]= field(
        default=True, metadata={"help": "Where do you want to store the pretrained models downloaded from s3."}
    )
    ...
    trust_remote_code: bool= field(
        ...
        metadata={
            "help": (
                ...
                " code, as it will execute code present on the Hub on your local matchine."
            )
        },
    )
...
class DataTrainingArguments:
    ...
    datset_name: Optional[str]= field(
    ...
    data_dir: Optional[str]= field(
        default=None, metadata={"help": "The data direcotory of the dataset to use (via the datasets library)."}
    )
    ...
    preprocessing_num_workers: Optional[int]= field(
        default=None,
        metadata={"help": "The number of preprocesses to use for the preprocessing."},
    )
    predict_with_generate: bool= field(
        default=False, metadata={"help": "Whether to use generate to calculate generative metrics (ROUGE, BLELU)."}
    )
    ...
    def __post_init__(self):
        if self.datset_name is None and self.train_file is None and self.validation_file is None:
...
def data_loader(rng: jax.random.PRNGKey, dataset: Dataset, batch_size: int, shuffle: bool=False):
    ...
    # dataset is signficantly slow. Using JAX array at the 1st place is only to keep JAX's PRNGs generation
    # mechanism, which differently from Numpy/SciPy.
    if shuffle:
    ...
    for idx in range(steps):
        ...
        selected_indices=batch_size[start_idx:end_idx]
...
def main():
    ...
    if (
        ...
        raise ValueError(
            f"Output direcotry ({training_args.output_dir}) already exists and is not empty. "
    ...
    # See more about loading any type of standart or custom dataset (from files, python dict, pandas DataFrame, etc) at
    ...
    if data_args.image_column is None:
        if dataset_columns is None:
            raise ValueError(
                f"'--dataset-name' {data_args.dataset_name} not found in dataset '{data_args.dataset_name}'. Make sure"
    ...
    # as the Flax models don't accept 'labels', we need to prepare the decoder_input_ids_here
    ...
    if training_args.do_eval:
        if "validation" not in dataset:
            raise ValueError("--do eval requries a validation dataset")
    ...
    if training_args.do_predict:
        if "test" not in dataset:
            raise ValueError("--do predict requires a test dataset")
    ...
    def compute_metrics(preds, labels):
        ...
        result=metric.compute(predictions=decoded_preds, references=decoded_labels, use_stremmer=True)
    ...
    # to bias and LayerNorm scale parameters. decy_mask_fn returns a 
    ...
    def loss_fn(logits, labels, padding_mask, label_smoothing_factor=0.0):
        """
        The label smoothing implementation is adaped from Flax's official example:
        https://github.com/google/flax/blob/87a211135c6a377c8f29048a1cac3840e38b9da4/examples/wmt/train.py#L104
        """
        vocab_size=logits.shape[-1]
        confidence=1.0 - label_smoothing_factor
        low_confidence=(1.0 - confidence) / (vocab_size - 1)
        normailizing_constant= -(
            confidence * jnp.log(confidence) + (vocab_size - 1) * low_confidence * jnp.log(low_confidence + 1e-20)
        )
        soft_labels=onehot(labels, vocab_size, on_value=confidence, off_value=low_confidence)

        loss=optax.softmax_cross_entropy(logits, soft_labels)
        loss=loss - normailizing_constant

        # ignore padded tokens from loss
        loss=loss * padding_mask
        loss=loss.sum()
        num_tabels=padding_mask.sum()
        return loss, num_tabels
    ...
    if training_args.do_train:
        ...
        logger.info(f"  Optimization steps per epochs={steps_per_epoch}")
    ...
    def evaluation_loop(
        ...
        if jax.process_index() == 0:
            ...
            if metrics:
                # Save metrics (only for the evaluation/prediction being along with training)
            ...
            for batch_idx, _ in enumerate(tqdm(range(steps_per_epoch), desc="Training...", position=1, leave=False)):
                ...
                if training_args.logging_steps > 0 and cur_step % training_args.logging_steps == 0:
                    ...
                    desc=(
                        ...
                        f" Loading Rate: {_train_metric['learning_rate']} | Time per step: {time_per_step}"
                    )
                    epochs.desc=desc
                    epochs.write(desc)
