...
class TrainingArguments:
    ...
    learning_rate: float=field(default=5e-5, metadata={"help": "The initail learning rate for AdamW."})
...
class DataTrainingArguments:
    ...
    dataset_config_name: Optional[str]=field(
        default=None, metadata={"help": "The configuration name of the dataset to use (via datasets library)."}
    )
    ...
    max_set_length: Optional[int]=field(
        default=None,
        metadata={
            "help": (
    ...
    permute_setence_ratio: float=field(
        default=1.0, metadata={"help": "Ratio of setences to be permuted in each document"}
    )
...
class FlaxDataCollatorForBartDenoisingLM:
    ...
    def __call__(self, examples: List[Dict[str, List[int]]])-> BatchEncoding:
        ...
        if self.permute_sentence_ratio > 0.0:
            batch["input_ids"]=self.permute_sentence_ratio(batch["input_ids"])
            do_permute=True
    ...
    def span_mask_tokens(self, input_ids, labels, do_permute):
        ...
        token_indies=np.argwhere(is_token_mask == 1)
        span_starts=np.random.permutation(token_indies.shape[0])[: span_lengths.shape[0]]
        # prepare mask
        masked_indices=np.array(token_indies[span_starts])
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file is not None:
            data_files["train"]=data_files.train_file
            extension=data_args.triain_file.split(".")[-1]
        ...
        if "validation" not in datasets.keys():
            datasets["validation"]=load_dataset(
                extension,
                data_files=data_files,
                split=f"train[:{data_args.validation_split_peercentage}%]",
                cache_dir=model_args.cache_dir,
                token=model_args.token,
                num_proc=data_args.preprocessing_num_workers,
            )
    ...
    if model_args.config_name:
        ...
        config=CONFIG_MAPPING[model_args.model_types]()
    ...
    # Use Punkt Tokenizer to device a document into a list of sentences
    nltk.download("punkt")
    ...
    # Main data processing function that will concatenate all texts from our dataset and generate chuncks of
    # max_seq_length.
    def group_texts(examples):
        # Concatenate all texts.
        concatenate_examples={k: list(chain(*examples[k])) for k in examples.keys()}
        total_lenth=len(concatenate_examples[list(examples.keys())[0]])
        # We drop the small reminder, we could add padding if the model supported it instead of this drop, you can
        # customize this part to your needs.
        if total_lenth >= max_seq_length:
            total_lenth=(total_lenth // max_seq_length) * max_seq_length
        # Split by chunks of max_len.
        result={
            k: [t[i: i + max_seq_length] for i in range(0, total_lenth, max_seq_length)]
            for k, t in concatenate_examples.items()
        }
        return result
    
    # Note that with 'batched=True', this map processes 1,000 texts together, so group_texts throwns away a 
    # reminder for each of those groups of 1,000 texts. You can adjust that batch_size here but a higher value
    # might be slower to preprocess.
    #
    # To speed up this part, we use multiprocessing. See the documentations of the map for more information:
    ...
    if has_tensorboard and jax.process_index() == 0:
        ...
        logger.warning(
            ...
            "Plaese run pip install tensorboard to enable."
        )
    ...
    decay_fn=optax.linear_schedule(
        ...
        end_value=0,
        train_steps=num_train_steps - training_args.warmup_steps,
    )
    ...
    # mask boolean with the same struncture as the parameters.
    ...
    if training_args.adafactor:
        ...
        optimzer=optax.adabelief(
            learning_rate=linear_decay_lr_schedule_fn,
        )
    else:
        optimzer=optax.adamw(
            learning_rate=linear_decay_lr_schedule_fn,
    ...
    # Setup train state
    state=train_state.TrainState.create(apply_fn=model.__call__, params=model.params, tx=optimzer)
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            state, train_metrics, dropout_rngs=p_train_step(state, model_inputs, dropout_rngs)
            train_metrics.append(train_metrics)
            ...
            if cur_step % training_args.logging_steps == 0 and cur_step > 0:
                ...
                epochs.write(
                    f"Step... ({cur_step} | Loss: {train_metrics['loss']}, Learning Rate:"
                    f" {train_metrics['learning_rate']})"
                )

                train_metrics=[]
            
            if cur_step % training_args.eval_step == 0 and cur_step > 0:
                # ======================== Evaluating =================================
    ...
    if training_args.do_eval:
        ...
        if jax.process_index() == 0:
            ...
            with open(path, "w") as f:
                json.dump(eval_metrics, f, indet=4, sort_keys=True)
