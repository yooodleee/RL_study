...
class Trainer:
    ...
    def update_weights(self, batch):
        ...
        obsesrvation_batch = (
            torch.tensor(numpy.array(obsesrvation_batch)).float().to(device)
        )
        ...
        current_value_loss, _, current_policy_loss = self.loss_funcion(
            value.squeeze(-1),
            reward.squeeze(-1),
            policy_logits,
            target_value[:, 0],
        ...
        for i in range(1, len(predictions)):
            ...
            # Cimpute priorities for the prioritized replay (See paper appendix Training)
            pred_value_scalar = (
                models.support_to_scalar(value, self.config.support_size)
                .detach()
                .cpu()
