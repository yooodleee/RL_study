...
class Node:
    ...
    def expand(
        ...
        """
        Expand a node using the value, reward and policy prediction obtained from the
        neural network.
        """
        ...
        policy_value = torch.softmax(
            torch.tensor([policy_logits[0][a] for a in actions]), dim=0
        ).tolist()
        policy = {a: policy_value[i] for i, a in enumerate(actions)}
        for action, p in policy.items():
            self.children[action] = Node(p)
    
    def add_exploration_noise(self, dirichlet_alpha, exploration_fraction):
        """
        At the start of each search, add dirichlet noise to the prior of the root to
...
class gameHistory:
    ...
    def store_search_statistics(self, root, action_space):
        ...
        if root is not None:
            sum_visit = sum(child.visit_count for child in root.children.values())
            self.child_visits.append(
                [
                    root.children[a].visit_count / sum_visit
